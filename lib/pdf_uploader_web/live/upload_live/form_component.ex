defmodule PdfUploaderWeb.UploadLive.FormComponent do
  use PdfUploaderWeb, :live_component
  ...
  def update(%{upload: upload} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Uploads.change_upload(upload))
     end)}
  end

  @impl true
  def handle_event("validate", %{"upload" => upload_params}, socket) do
    changeset = Uploads.change_upload(socket.assigns.upload, upload_params)
